...
import com.example.model.FolderAnalytics
...
object FileManager {
  ...
  fun detectUsbDrive(): UsbDriveDetails? {
    return try {
      val storageDir = File("/storage")
      if (storageDir.exists() && storageDir.isDirectory) {
        val children = storageDir.listFiles() ?: emptyArray()
        for (child in children) {
          val name = child.name
          if (child.isDirectory && child.canRead() && name != "emulated" && name != "self" && name != "knox-emulated") {
            ...
            return UsbDriveDetails(
              name = if (name.matches(Regex("[A-Za-z0-9]{4}-[A-Za-z0-9]{4}"))) "USB Volume ($name)" else "USB Storage ($name)",
              path = child,
  ...
  suspend fun delete(target: File): Result<Boolean> = withContext(Dispatchers.IO) {
    try {
      ...
      if (success) {
        ...
        // Fallback retry using Kotlin deleteRecursively
        val fallback = if (target.isDirectory) target.deleteRecursively() else target.delete()
        if (fallback || !target.exists()) Result.success(true) else Result.failure(IOException("Failed to delete ${target.name}"))
      }
    } catch (e: Exception) {
      Result.failure(e)
    }
